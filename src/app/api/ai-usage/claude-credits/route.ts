...
import {
    addClaudeCreditPurchase,
    correctClaudeCreditBalance,
    isValidDateKey,
    removeClaudeCreditPurchase,
    toMinorUnits,
} from "@/lib/ai-usage/claude-credit-ledger"

/**
 * Claudeのクレジット購入・残高の補正を記録する（#252）。
 *
 ...
 * - `{ kind: "balance", amount: 11.19 }` — Claude.aiの画面に出ている残高で推定をやり直す
 */
export async function POST(request: NextRequest) {
    ...
    const amountMinor = typeof amount === "number" ? toMinorUnits(amount) : null
    if (amountMinor === null) {
    ...
    if (kind === "purchase") {
        ...
        await addClaudeCreditPurchase(date, amountMinor)
    ...
    if (kind === "balance") {
        ...
        await getAiUsageSnapshot().catch((error) => {
            console.error("Claude credits: 補正前の使用状況の取得に失敗", error)
        })
        await correctClaudeCreditBalance(amountMinor)
...
export async function DELETE(request: NextRequest) {
    ...
    const removed = await removeClaudeCreditPurchase(id)
    if (!removed) {
