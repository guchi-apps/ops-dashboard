...
| tmux | セッションの一覧。稼働中・入力待ち・待機中・放置の別、実行中コマンド、作業ディレクトリ、最終活動 |
...
状態は各ペインの実行コマンドと、tmuxの最終活動時刻（`session_activity`）の2つから判定する（[issue #40](https://github.com/guchi-apps/ops-dashboard/issues/40)）。
デタッチしたまま裏でclaudeが走っているのが普通の使い方で、アタッチの有無では「いま作業が動いているか」を判断できないため。

さらに、claudeは入力待ちの間もプロセスが残り続けるため、コマンドの有無だけでは処理中のセッションと手が止まっているセッションを分けられない。
処理中は画面が描き変わり続けて `session_activity` が更新されるのに対し、プロンプトで止まると更新されなくなる。これを手がかりに両者を分けている。

| 状態 | 判定 |
| --- | --- |
| 稼働中 | シェル（bash・zsh等）以外のコマンドが動いていて、最終活動が1分以内 |
| 入力待ち | シェル以外のコマンドは動いているが、最終活動から1分以上 画面が止まっている |
| 待機中 | シェルだけで止まっている |
| 放置 | デタッチのまま24時間以上 活動がない |

**入力待ちの判定だけは、画面の現在時刻ではなくレポートの収集時刻（`collectedAt`）を基準に測る。**
画面の現在時刻で測ると、次のレポートが届くまでのあいだ経過だけが伸びていくため、しきい値が送信間隔（1分）と同じでは、
処理中のセッションもレポート到着の直前に必ずしきい値を越えてしまい、1分ごとに状態がちらつく。
収集時刻はホスト自身の時計であり、最終活動時刻と同じ時計のため、ホストとサーバーの時計ずれも判定に入らない。

この基準のぶん、実際に手が止まってから画面に出るまでは最大で送信間隔（1分）ぶん遅れる。

`commands` / `busy` / `path` / `lastActivityAt` はいずれも任意の項目のため、エージェントを更新していないホストからのレポートもそのまま受け取れる（その場合はアタッチの有無で稼働中を判定し、入力待ちの切り分けは行わない）。

セッション名は `<リポジトリ名>-issue-<番号>` に統一されているため（issue-deck の `scripts/start-issue.sh`）、名前から Issue を特定して**GitHubのIssueへのリンク**にしている。
飛び先をissue-deckにしていないのは、issue-deck側にIssue単体を指すURLが無いため（画面が1枚で、選択状態をURLに持たない）。
owner は `NEXT_PUBLIC_GH_OWNER` から取り、**未設定ならリンクにしない**（組織名を当て推量で埋めると、存在しないURLへ飛ばすことになる）。書式に合わない名前のセッション（手元から直接起動したもの）も文字のまま出す。
