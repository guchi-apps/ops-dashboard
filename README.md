...
| 入力待ち | フックの最後のイベントが `Stop` 以外（承認・質問で人を待っている）、または画面が60秒以上 止まっている |
| 待機中 | シェルだけで止まっている |
...
画面が止まっているかどうかは、フックが届かないセッション（手で立てたもの）と、`Stop` を出したあと何も起きていないセッションのための代替手段。出力の無いビルドを「入力待ち」と誤判定する、スピナーが回っているだけのセッションを「稼働中」と読み続ける、といった外し方をするため、あくまで補助として使う。

`Stop`（応答の終了）だけでは入力待ちと断定しない。応答を終えた直後に次の作業へ入っていることがあるため。
